#!/usr/bin/env python3
"""Operate the preregistered AFM-II MnO strict v3r2 zero-shift preflight.
...
# The prior root contains job 320, which was launched without the locked OpenMP
# setting.  Preserve it as excluded forensic evidence; only this fresh root can
# be admitted by the repaired foreground launcher.
ADMISSIBLE_REPLICA_ROOT = CAMPAIGN / "results" / "calibration-replicas-admissible-foreground-v3"
...
def _verify_software_lock() -> None:
    lock_path = CAMPAIGN / "locks/software-lock.json"
    lock = json.loads(lock_path.read_text(encoding="utf-8"))
    wheel = CAMPAIGN / lock["wheel"]["path"]
    _require(wheel.is_file() and _sha(wheel) == lock["wheel"]["sha256"], "frozen wheel hash mismatch")
    for relative, expected in lock["files"].items():
        path = ROOT / relative
        _require(path.is_file() and _sha(path) == expected, f"software lock mismatch: {relative}")


def verify() -> dict[str, object]:
    _verify_software_lock()
...
def _plan() -> list[dict[str, object]]:
    return [{"replica_id": f"recovery-{index:02d}", "slurm": {"nodes": 1, "tasks": 4,
...
def _frozen_wheel_wrapper(runtime: Path, replica_id: str) -> str:
    """Use the immutable wheel and remove ambient PYTHONPATH from a job."""
    wheel = CAMPAIGN / "software/siestaflow_hubbard-0.1.0-py3-none-any.whl"
    python = Path(sys.executable)
    worker = ROOT / "tools/run_mno_afmii_strict_v3r2.py"
    parts = [
        "set -eu",
        ...
        "env -u PYTHONPATH " + " ".join(shlex.quote(str(item)) for item in [runtime / "bin" / "python", worker, "preflight-worker", replica_id]),
    ]
    return "; ".join(parts)


def _foreground_commands() -> list[list[str]]:
    """Build five independent, blocking four-rank allocations."""
    from siestaflow_hubbard.execution.slurm_foreground import four_rank_foreground_command
    return [four_rank_foreground_command(_frozen_wheel_wrapper(
        ADMISSIBLE_REPLICA_ROOT / f".recovery-{index:02d}-runtime", f"recovery-{index:02d}"))
        for index in range(1, 6)]
...
def bind() -> dict[str, object]:
    ...
    receipts = [receipt_root / f"recovery-{index:02d}" / "replica-result.json" for index in range(1, 6)]
    ...
    for record in records:
        _require(
            len(record["values"]) == len(roles) and set(record["values"]) == set(roles),
            "calibration receipt controls do not match lock",
        )
...
def main() -> int:
    ...
    try:
        if args.command == "verify": result = verify()
            ...
            verify(); commands = _foreground_commands()
            if args.dry_run:
                result = {"status": "DRY_RUN", "jobs": _plan(), "commands": commands}
            else:
                from siestaflow_hubbard.execution.slurm_foreground import submit_four_rank_foreground
                submitted: dict[str, str] = {}
                for index, command in enumerate(commands, start=1):
                    # The helper owns command construction; invocation is
                    # recovered solely to keep one auditable submit call.
                    completed = submit_four_rank_foreground(command[-1], cwd=ROOT)
                    submitted[f"recovery-{index:02d}"] = _parse_sbatch_job_id(completed.stdout)
                result = {"status": "SUBMITTED_AND_COMPLETED", "slurm_job_ids": submitted,
                          "bind_next": "bind-calibration"}
