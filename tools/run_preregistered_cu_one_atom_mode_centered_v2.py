...
from siestaflow_hubbard.domain.occupation_noise_calibration import (
    ...
    derive_mode_centered_occupation_noise,
    load_strict_json,
...
CAMPAIGN = ROOT / "campaigns" / "cu_one_atom_mode_centered_lr_v2"
...
def _verify_locks() -> dict[str, Any]:
    ...
    required_software = {
        "tools/run_preregistered_cu_one_atom_mode_centered_v2.py", "tools/run_cu_mode_centered_calibration_replica_v2.py",
        "tools/run_siesta542_openmpi_slurm_mode_centered_v2.py",
        ...
        "schemas/campaign/occupation_noise_calibration_lock_v2.schema.json",
        "schemas/campaign/occupation_noise_calibration_result_v2.schema.json",
        "campaigns/cu_one_atom_mode_centered_lr_v2/inputs/reference.fdf",
        "examples/tmo_campaigns/Cu1.psml",
        "campaigns/cu_one_atom_mode_centered_lr_v2/locks/calibration-methodology-lock.json",
        "campaigns/cu_one_atom_mode_centered_lr_v2/locks/response-methodology-lock.json",
        "pyproject.toml",
    }
...
def verify_package(*, require_result: bool) -> dict[str, Any]:
    ...
    _validate_json_schema(calibration, ROOT / "schemas/campaign/occupation_noise_calibration_lock_v2.schema.json",
    ...
    result_path = CAMPAIGN / "results-v2/calibration-result.json"
    ...
    if require_result or result_path.exists():
        ...
        result = _load_strict("results-v2/calibration-result.json")
        _validate_json_schema(result, ROOT / "schemas/campaign/occupation_noise_calibration_result_v2.schema.json",
                              "calibration result")
        ...
        evidence_root = (CAMPAIGN / "results-v2" / "calibration-replicas").resolve()
        ...
        derived = derive_mode_centered_occupation_noise(occupations, policy)
...
def bind_calibration(
    ...
    *,
    ...
    """
    result_output = CAMPAIGN / "results-v2/calibration-result.json"
    if _write_result:
    ...
    for value in replica_paths:
        ...
        allowed = (CAMPAIGN / "results-v2" / "calibration-replicas").resolve()
    ...
    result = derive_mode_centered_occupation_noise(values, policy)
...
def submit_calibration(*, dry_run: bool) -> dict[str, Any]:
    ...
    verify_package(require_result=False)
    software = _verify_locks()
    worker = ROOT / "tools/run_cu_mode_centered_calibration_replica_v2.py"
    result_root = CAMPAIGN / "results-v2/calibration-replicas"
    commands = []
...
def submit_response(*, dry_run: bool) -> dict[str, Any]:
    ...
    evidence = CAMPAIGN / "results-v2/response-campaign-attempt-2"
    if evidence.exists():
        raise ValueError("refusing to reuse response evidence directory")
    runner = ROOT / "tools/run_siesta542_openmpi_slurm_mode_centered_v2.py"
    software = _verify_locks()
    result_path, lock_path, software_lock = (CAMPAIGN / "results-v2/calibration-result.json",
