...
PREREGISTERED_CAMPAIGN = ROOT / "campaigns" / "cu_one_atom_noise_calibrated_lr_v1"
...
def _response_nodes() -> tuple[LRDagNode, ...]:
    ...
    # Alpha zero is represented by the one validated reference, as required by
    # adaptive_alpha_perturbations; it is not materialized as an ambiguous run.
...
def _external_occupation_noise() -> tuple[float, dict[str, object]] | None:
    ...
    expected_result_path = (PREREGISTERED_CAMPAIGN / "results/calibration-result.json").resolve()
    ...
    admission = subprocess.run(
        [sys.executable, str(ROOT / "tools/run_preregistered_cu_one_atom_campaign.py"), "response-admission"],
...
def main() -> int:
    ...
    try:
        ...
        response_lock_path = ROOT / "campaigns" / "cu_one_atom_noise_calibrated_lr_v1" / "locks" / "response-methodology-lock.json"
        ...
        ref_obs = _reference_observation(by_node["reference"])
        observations = [_response_observation(node, by_node[node.node_id], bare_profile) for node in responses]
        observations.extend([{"node_id": "reference_as_alpha_zero", "mode": mode, "alpha_ev": 0.0,
                              "role": "validated_reference_only", **ref_obs,
                              "output_sha256": sha256_file(by_node["reference"])} for mode in ("BARE", "SCREENED")])
        ...
        report = "# SIESTA 5.4.2 full centred Cu LR campaign\n\nAll 13 execution nodes passed the admitted canonical route: one reference and six BARE plus six SCREENED responses at ±0.025, ±0.050 and ±0.100 eV. Alpha zero is the validated reference, as required by the adaptive-alpha DAG contract.\n\nThe machine-readable alpha gate records whether the external hash-bound occupation-noise calibration admitted the common window, direct inverses, and U. No response observation is used to establish that bound.\n"
