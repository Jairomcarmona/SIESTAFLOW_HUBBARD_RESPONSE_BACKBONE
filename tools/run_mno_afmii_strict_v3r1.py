#!/usr/bin/env python3
"""Operate the preregistered AFM-II MnO strict v3r1 zero-shift preflight.
...
CAMPAIGN = ROOT / "campaigns" / "mno_afmii_strict_lr_v3r1"
...
def _plan() -> list[dict[str, object]]:
    return [{"replica_id": f"restart-{index:02d}", "slurm": {"nodes": 1, "tasks": 4,
...
def bind() -> dict[str, object]:
    ...
    receipts = [receipt_root / f"restart-{index:02d}" / "replica-result.json" for index in range(1, 6)]
